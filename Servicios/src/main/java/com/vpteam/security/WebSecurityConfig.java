...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter 
{
	...
	protected void configure(HttpSecurity http) throws Exception
	{
		  http
          .authorizeRequests()
          .antMatchers("/dashboard_oferente", "/new_job").access("hasRole('ROLE_OFERENTE')")
          .antMatchers("/dasboard", "/insertarPersona").access("hasRole('ROLE_USUARIO')")
              .antMatchers("/", "/home", "/css/**", "/handlebars/**", "/js/**", "/fragments/**").permitAll()
              .anyRequest().authenticated()
              .and()
          .formLogin()
              .loginPage("/login").successHandler(new AuthConfig())
              .permitAll()
              .and()
